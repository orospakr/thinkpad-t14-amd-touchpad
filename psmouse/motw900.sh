...
timeout --foreground 900 evtest /dev/input/$n 2>/dev/null | grep -a --line-buffered 'SYN_REPORT' | grep --line-buffered -o 'time [0-9.]*' | awk '{print $2; fflush()}' | { read first || exit; echo "$first"; end=$(echo "$first + 20" | bc); while read t; do echo "$t"; [ "$(echo "$t > $end" | bc)" = 1 ] && break; done; } > $OUT.syn
